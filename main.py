...
def new_cust():
    ...
    customer = bank_ac(name=name, mobile=mobile, initial_depo=initial_depo, pin=pin)
    customer_dict[customer.ac_no] = customer  # acct. no. stored as key and oject as value
    mobile_acc_link[customer.mobile] = customer.ac_no  # mobile no. linked
    print('New User Created!')
    print(f'Welcome {customer.name} to Corporate Bank. {customer.ac_no} is your account number')


def login():
    account_no = int(input('Enter your Account Number: '))
    account_pin = int(input('Enter your Account PIN: '))
    if account_no not in customer_dict.keys() or account_pin != customer_dict[account_no].pin:
        print('Account either not exist or the pin is wrong')
        return
    else:
        print(f'\n{customer_dict[account_no].name} Logged in')
        customer_dict[account_no].basic_detail()
    while True:
...
Press 4 to log out\n''')
        if user_input1 == '1':
            ...
            customer_dict[account_no].withdrawal()
        elif user_input1 == '3':
...
Press 4 for exit\n''')
    ...
    if user_input1 == '1':
        ...
        print('There currently', bank_ac.no_0f_customer, 'customers in Corporate bank.')
    elif user_input1 == '4':
        print('Exited')
        break
    else:
